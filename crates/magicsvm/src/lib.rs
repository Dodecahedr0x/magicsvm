...
`magicsvm` is a fast and lightweight library for testing Solana programs.
...
fn test_logging() {
    ...
    let bytes = include_bytes!("../../node-magicsvm/program_bytes/spl_example_logging.so");
...
fn test_set_clock() {
    ...
    let mut svm = LiteSVM::new();
    let bytes = include_bytes!("../../node-magicsvm/program_bytes/magicsvm_clock_example.so");
    svm.add_program(program_id, bytes);
    let payer = Keypair::new();
